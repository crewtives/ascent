describe("ClassicQuestLogReader", function()
  ...
  local function stubQuestLog(fixtureEntries, initialSelection)
    ...
    -- Both invocation forms: no argument answers for whichever entry is
    -- currently selected (the one the reader uses); with a questId it answers
    -- for that quest directly, regardless of selection.
    ...
    -- Trusted by default: most tests are about the shape checks. The provenance
    -- test overrides this before constructing the reader.
  ...
  local function load()
    local ns = AscentTest.loadWith("core/model/", "core/port/", "adapter/inbound/GlobalStringPattern.lua",
      "adapter/outbound/QuestLogShape.lua", "adapter/outbound/ClassicQuestLogReader.lua")
    QuestXpOrigin = ns.core.QuestXpOrigin
    reader = ns.adapter.ClassicQuestLogReader.new()
  end
  ...
  -- With a logger, the sweep also dumps the provenance check and both invocation
  -- forms per quest, without changing what is returned.
  describe("quest reward diagnostics (optional logger)", function()
    ...
    it("flags a disagreement between the two forms instead of hiding it", function()
      ...
      stubQuestLog({ { questId = 101, level = 10, isHeader = false, reward = 250 } }, 0)
      -- Reassigned before load(): the reader captures this reference at
      -- construction rather than reading the global by name on every call.
      _G.GetQuestLogRewardXP = function(questId)
        if questId ~= nil then
  ...
  -- The debug log is a 500-line ring shared with roughly three lines per kill, so
  -- a sweep taken an hour into a session would be gone before anyone read it; the
  -- evidence file keeps it as one sample.
