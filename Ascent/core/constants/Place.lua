...
--
-- A dimension of its own, orthogonal to XpSource (D39): a kobold killed inside a
-- dungeon is still a mob kill, and a quest turned in down there is still a turn-in.
-- Making "dungeon" a source would have forced every gain to choose between two
-- facts it already knows, and would have changed the meaning of every `mob_kill`
-- already on disk without anything failing to say so.
--
...
-- place that happened to be known (D42).
