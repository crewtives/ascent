...
ns.core.EventTopic = Frozen.enum("EventTopic", {
  ...
  -- Published rather than only logged, so the exact text the client printed can
  -- be recorded and read afterwards.
  ...
  -- combat log that has one on each side. Separate from the damage topics because
  -- being fought is not being hurt: a creature that swung and missed, or one the
  -- player never hit back, is still in the pull.
  ...
  QUEST_REWARD_SEEN = "quest_reward_seen", -- reward read from the open quest dialogue
...
ns.core.WowEvent = Frozen.enum("WowEvent", {
  ...
  -- The only reliable source of a nameplate's unit token: the frames that
  -- C_NamePlate.GetNamePlates() returns can carry none.
...
ns.core.CombatLogSubevent = Frozen.enum("CombatLogSubevent", {
  ...
  -- A hit that landed on a shield. Carried for who is in the fight and nothing
  -- else, so it has no handler.
  SPELL_ABSORBED       = "SPELL_ABSORBED",
  -- A creature winding up a spell at this character. Carried for the same reason
  -- and also without a handler: it is the earliest thing the combat log says
  -- about a caster.
