...
--
-- Two marks carry meaning beyond their value and are worth understanding:
--
...
--                 because the gap is seeded into UNKNOWN -- but the level did not
--                 get an honest accounting and the panel says so.
--   seededXp      how much of that UNKNOWN is the seed itself. `partial` says the
--                 accounting is incomplete; this says by how much, which is a
--                 different question and the only one that separates experience
--                 nobody watched from experience that was watched and could not be
--                 attributed. Both live in UNKNOWN and are indistinguishable
--                 without it -- a surface showing "unclassified" is otherwise
--                 blaming the addon for the larger half.
--
...
--                 defaulting it to zero would claim every one of those points was
--                 observed and unattributed. Zero means "seeded nothing", nil
--                 means "never recorded which".
...
function LevelRecord:reset(level, startedAt)
  ...
  -- over the same experience the sources already account for (D39), never a
  -- partition of it -- which is why it has its own map and not a field on a gain.
  self.places = {}
...
-- The invariant the whole addon exists to keep: what the sources add up to must
-- equal what was actually earned. Exposed so tests and diagnostics can assert it
...
function LevelRecord:placeEntry(key)
  ...
  elseif entry.key.name == nil and key.name ~= nil then
    -- The name arrives late more often than it looks. The client answers a map id
    -- before it answers zone text -- during a loading screen `GetZoneText()` is the
    -- empty string while `C_Map` already knows where the character is -- so the
    -- first sighting of a place routinely identifies it without naming it. Keeping
    -- that first key forever left the zone a player spent a whole level in reading
    -- as the reserved "somewhere we could not name", which is a different thing
    -- entirely. The identity never changes here: only the half that was missing.
    entry.key = key
  end
  return entry
end
...
-- Everything earned in places of one kind, which is the question "how much of
-- this level came from dungeons" -- answerable without naming a single one of
-- them, which is the whole reason the kind travels inside the key.
...
-- reader. Conflating them let the diagnostic print "1000 of 1000 experience
-- placed" directly above "with no place the client could name: 1000 (100%)".
...
-- ---------------------------------------------------------------------------
-- Combat efficiency (6.7) -- derived, not accumulated. Out-of-combat time is
-- derived here too, the same way: playedSeconds minus combatSeconds, rather than
-- a third counter CombatTimeCollector would have to keep in sync with the other
-- two (D23).
-- ---------------------------------------------------------------------------
...
-- Nil with no combat time at all, per the spec's own "reported as unavailable
-- instead of producing an error" -- a level spent entirely on quests has nothing
-- to divide by.
...
--     difference between four megabytes and under one. It is what makes keeping
--     every level's full detail affordable instead of throwing it away, which is the
--     one thing a leveling addon must not do.
-- ---------------------------------------------------------------------------
...
-- is not mentioned by any line written before it existed. Columns would have made
-- the position the meaning, so inserting a source anywhere but the end would
-- silently re-read every place ever written. The pairs are walked in the
...
local function unpackPlaceSources(fields)
  ...
  while true do
    ...
    -- everything behind it. The pairs encoding was chosen over columns precisely
    -- so a future source costs no migration, and `packPlace` writes them in the
    -- enumeration's sorted order -- so a new source lands in the MIDDLE of the
    -- tail, and stopping at it would wipe every pair an older build could still
    -- have read, then write the loss back to disk at the next logout.
    index = index + 2
  end
end
...
-- The only key of `metrics` that ever holds a live object (D23, D20): every other
-- collector writes a plain table straight into its own slot, so `Stored.plainCopy`
-- reaches those untouched -- this is the one substitution needed before it does,
-- swapping the live CombatSummary for its own toStored() so plainCopy never meets
-- a metatable and raises "give it a toStored() of its own", which is exactly what
-- this is.
...
function LevelRecord:toStored()
  return {
    ...
    -- object; everything else still raises through Stored.plainCopy if a future
    -- collector hands this a metatable by mistake, because that would be the
    -- addon's own bug and the file would otherwise swallow it in silence and hand
    -- back half a record next login.
...
-- The guarantee the addon exists to make is that the sources add up to the level's
-- total, and a file the player can edit -- or a logout cut off halfway -- can arrive
-- disagreeing with itself. The disagreement is absorbed the same way a live gain
-- nobody could explain is: into UNKNOWN. When the breakdown claims MORE than the
-- total, the breakdown is the more detailed record of the two and the total yields.
...
function LevelRecord.restore(stored)
  ...
  -- could not, when the truth is that it never tried (migration plan, D42).
  ...
  -- two lines and one key. Assigning would have deleted the first one's
  -- experience and its time without a word.
